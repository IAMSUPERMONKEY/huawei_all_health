...
public class AutoRecorderBackgroundService extends Service {
    ...
    private static int configSmallIcon(Bundle bundle, Context context) {
        ...
        resourceId = value != null
            ? res.getIdentifier(value, AutoRecorderConstants.MIPMAP, packageName)
            : res.getIdentifier("ic_notification", AutoRecorderConstants.MIPMAP, packageName);

        if (resourceId == 0) {
            resourceId = res.getIdentifier("ic_launcher", AutoRecorderConstants.MIPMAP, packageName);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent.getAction() != null) {
            ...
            } else if (intent.getAction().equals(AutoRecorderConstants.STOP_BACKGROUND_SERVICE_ACTION)) {
                stopForeground(true);
                stopSelf();
            }
        }
    ...
    private void getNotification(Bundle bundle) {
        ...
        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(context,
            ...
            .setContentTitle(getBundleString(bundle, AutoRecorderConstants.TITLE))
            .setContentText(getBundleString(bundle, AutoRecorderConstants.TEXT))
            .setContentIntent(pendingIntent)
            .setUsesChronometer(getBundleBoolean(bundle, AutoRecorderConstants.CHRONOMETER))
            .setCategory(NotificationCompat.CATEGORY_SERVICE)
            .setOngoing(true);
        if (hasValue(bundle, AutoRecorderConstants.TICKER)) {
            notificationBuilder.setTicker(bundle.getString(AutoRecorderConstants.TICKER));
        }
        if (hasValue(bundle, AutoRecorderConstants.SUB_TEXT)) {
            notificationBuilder.setSubText(bundle.getString(AutoRecorderConstants.SUB_TEXT));
        }
